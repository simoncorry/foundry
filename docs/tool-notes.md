...
Claude Code reads `.claude/commands/*.md` (our generated byte-copies) and pulls the shared rules in through CLAUDE.md's one-line `@AGENTS.md` import. Note the official status: Anthropic merged commands into skills, so the commands folder is documented as legacy, kept working. Foundry ships it anyway, deliberately, because keeping the Claude copies byte-identical to the Cursor source is what lets the check prove the three shapes match; a skills-based Claude shape is the documented later option if the legacy path ever actually goes away.

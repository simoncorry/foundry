<!-- Budget: ~2,000 tokens (~8 KB). This file loads at the start of every session, so every line here is a tax on every session. The executable limit lives in scripts/check-context-budgets.js; if an addition crosses it, something else moves out to docs/ first. The project this grew from let its rules file double past its own declared budget; the budget line you're reading exists so that doesn't happen here. -->

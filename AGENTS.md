...
Everything written for a human reads like a person wrote it: plain words, short sentences, no jargon stacks, no em dashes. Before sending a substantive response, pipe the draft through `node scripts/voice-gate.js` and rewrite what it flags. The gate only knows the phrases in `scripts/phrase-list.json`; wrap-up's jargon step grows that list every session, so it keeps pace with the jargon you actually produce. For vocabulary the list doesn't know yet, the test is: would someone outside the codebase follow this sentence? If not, rewrite it.
